// ============================================================
// SciVerse Academy — COMPLETE BACKEND (ALL FEATURES)
...
async function ensureTables(db) {
  ...
  await db.prepare(`
    ...
    VALUES ('Introduction to Science', 'Learn the fundamentals of science with expert-led lessons.', '🔬', 1, 1)
  `).run();

  // Seed sample subject
  await db.prepare(`
    INSERT OR IGNORE INTO subjects (course_id, title, description, sort_order)
    VALUES (1, 'Chapter 1: Basics of Science', 'Introduction to basic scientific concepts', 0)
  `).run();
...
async function handleAdmin(method, path, body, db, user, request) {
  ...
  if (method === 'POST' && path === '/admin/subjects') {
    ...
    await db.prepare('INSERT INTO audit_logs (user_id, action, ip_address, user_agent) VALUES (?, ?, ?, ?)').bind(user.id, `created_subject_${result.meta.last_row_id}`, request.headers.get('CF-Connecting-IP') || '', request.headers.get('User-Agent') || '').run();
  ...
  if (method === 'DELETE' && path.match(/^\/admin\/subjects\/\d+$/)) {
    ...
    return json({ message: 'Subject and all lectures deleted.' });
  }
  ...
  if (method === 'POST' && path === '/admin/lectures') {
    ...
    await db.prepare('INSERT INTO audit_logs (user_id, action, ip_address, user_agent) VALUES (?, ?, ?, ?)').bind(user.id, `created_lecture_${result.meta.last_row_id}`, request.headers.get('CF-Connecting-IP') || '', request.headers.get('User-Agent') || '').run();
  ...
  if (method === 'POST' && path === '/admin/resources') {
    ...
    await db.prepare('INSERT INTO audit_logs (user_id, action, ip_address, user_agent) VALUES (?, ?, ?, ?)').bind(user.id, `created_resource_${result.meta.last_row_id}`, request.headers.get('CF-Connecting-IP') || '', request.headers.get('User-Agent') || '').run();
